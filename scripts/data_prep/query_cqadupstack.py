...
class Subforum():
	...
	if not self.cutoffdate:
        ...
        print ("Getting ordered list of posts...")
	...
	trainfile1 = codecs.open(outputdir + '/' + self.cat + '_trainpairs_01.txt', 'w', 'utf-8')
        trainfile2 = codecs.open(outputdir + '/' + self.cat + '_trainpairs_02.txt', 'w', 'utf-8')
        trainfile3 = codecs.open(outputdir + '/' + self.cat + '_trainpairs_03.txt', 'w', 'utf-8')
        trainfile4 = codecs.open(outputdir + '/' + self.cat + '_trainpairs_04.txt', 'w', 'utf-8')
        trainfile5 = codecs.open(outputdir + '/' + self.cat + '_trainpairs_05.txt', 'w', 'utf-8')
        trainfile6 = codecs.open(outputdir + '/' + self.cat + '_trainpairs_06.txt', 'w', 'utf-8')
        trainfile7 = codecs.open(outputdir + '/' + self.cat + '_trainpairs_07.txt', 'w', 'utf-8')
        trainfile8 = codecs.open(outputdir + '/' + self.cat + '_trainpairs_08.txt', 'w', 'utf-8')
        trainfile9 = codecs.open(outputdir + '/' + self.cat + '_trainpairs_09.txt', 'w', 'utf-8')
        trainfile10 = codecs.open(outputdir + '/' + self.cat + '_trainpairs_10.txt', 'w', 'utf-8')
	...
	traindups = 0
        for i,postid in tqdm.tqdm(enumerate(trainposts)):
            dups = self.get_duplicates(postid)
            combine_with = trainposts[i+1:] # combine each post with older posts.
            for otherpostid in combine_with:
                if otherpostid in dups:
		    filenrs[dupindex].write('1\t' + self.perform_cleaning(self.get_posttitle(postid)) + '\t' + self.perform_cleaning(self.get_posttitle(otherpostid)) + '\t'+postid+'-'+otherpostid+'\n') # 1 = duplicate pair
		    if dupindex == 9:
                        dupindex = 0
                    else:
                        dupindex += 1
		    ...
		    filenrs[nondupindex].write('0\t' + self.perform_cleaning(self.get_posttitle(postid)) + '\t' + self.perform_cleaning(self.get_posttitle(otherpostid)) + '\t'+postid+'-'+otherpostid+'\n') # 0 = non-duplicate pair
		    if nondupindex == 9:
			nondupindex = 0
		    else:
		        nondupindex += 1
